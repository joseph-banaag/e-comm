      ...
      <div className="flex items-center gap-2 lg:gap-5 flex-wrap">
        {subNavLinks.map((link) => (
          <Link
            key={link.label}
            href={link.route}
            className={`text-nowrap border border-gray-200 rounded-4xl px-2 py-0.5 bg-gray-100 text-[#19191a] transition-all duration-300 hover:bg-gray-700/70 hover:text-[#e2e4e8] text-shadow-lg shadow-lg`}
          >
            {link.label}
          </Link>
        ))}
